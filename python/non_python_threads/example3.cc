...
int main(int argc, char *argv[]) {
    ...
    boost::asio::post(default_pool, [&gstate]() {
        gstate = init_python_thread("default_pool");
    });

    boost::asio::post(custom_pool, [&gstate]() {
        gstate = init_python_thread("custom_pool");
    });
    ...
    boost::asio::post(default_pool, [&gstate]() {
        release_gstate(gstate, "default_pool");
    });

    boost::asio::post(custom_pool, [&gstate]() {
        release_gstate(gstate, "custom_pool");
    });
